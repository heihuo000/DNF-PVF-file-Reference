function checkExecutableSkill_Zskill00(obj)  
{
	...
	if (isUse) 
	{
		obj.getVar().setInt(0,0);//設定初始數值為0
...
function onEndCurrentAni_Zskill00(obj)
{
	...
	local HitCount = obj.getVar().getInt(0);//獲取當前值
	obj.getVar().setInt(0,HitCount + 1);//每完成一次ANI動作數值加1
	...
	{
		obj.sq_IntVectClear();//清除向量
		obj.sq_IntVectPush(0);//將向量寫入值“0” （*解釋：返回到substate = 0，重復第一次的動作）
		obj.sq_AddSetStatePacket(STATE_ZSKILL00, STATE_PRIORITY_IGNORE_FORCE, true);
	}
	else if(substate == 4)
	{
...
("Character/Mage/Effect/Animation/ATIceSword/x.ani");//在演出動畫結束時將受擊動畫指向到空的文件
		obj.sq_AddSetStatePacket(STATE_STAND, STATE_PRIORITY_USER, false);//發送站立狀態 （*解釋：中斷重復動作）
	}
}
...
function onProc_Zskill00(obj)
{
	...
	local maxHitCount = obj.sq_GetIntData(SKILL_ZSKILL00, 0); // 讀取SKL靜態數據“0”
	...
	if(HitCount >= maxHitCount)//當設置技能的“substate”狀態 等于 = 0 時，執行下列向量寫入
		...
		if(substate <= 3)
		{
			...
			obj.sq_IntVectClear();//清除向量
			obj.sq_IntVectPush(4);//將向量寫入值“0” （*解釋：返回到substate = 0，重復第一次的動作）
			obj.sq_AddSetStatePacket(STATE_ZSKILL00, STATE_PRIORITY_IGNORE_FORCE, true);
			}
		}
}
